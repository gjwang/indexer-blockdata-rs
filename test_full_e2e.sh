...
docker exec -i scylla cqlsh -e "TRUNCATE settlement.ledger_events;" 2>/dev/null || true
rm -f /tmp/*.log
...
docker exec -i scylla cqlsh -e "SELECT user_id, asset_id, available, frozen, version FROM settlement.user_balances WHERE user_id IN (1001, 1002, 1003) ALLOW FILTERING;" 2>/dev/null

echo ""
echo "3. All User Balances Count:"
docker exec -i scylla cqlsh -e "SELECT COUNT(*) FROM settlement.user_balances;" 2>/dev/null
