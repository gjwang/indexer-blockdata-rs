...
echo ""
echo "=== Step 9: Sending test orders (30 seconds) ==="
...
grep -c "DEPOSIT_CONSUMED" logs/settlement.log 2>/dev/null || echo "0"
echo -n "  Trade balance updates: "
grep -c "PROGRESS.*trades=" logs/settlement.log 2>/dev/null || echo "0"
echo -n "  Version conflicts: "
grep -c "Balance update skipped" logs/settlement.log 2>/dev/null || echo "0"
echo -n "  Balance errors: "
grep -c "Failed to update balances" logs/settlement.log 2>/dev/null || echo "0"

echo ""
echo "=== Sample Logs ==="
echo "Deposit updates:"
grep "DEPOSIT_CONSUMED" logs/settlement.log 2>/dev/null | head -3 || echo "None"
echo ""
echo "Trade updates:"
grep "PROGRESS.*trades=" logs/settlement.log 2>/dev/null | head -3 || echo "None"
