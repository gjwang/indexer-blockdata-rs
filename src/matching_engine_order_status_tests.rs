...
mod tests {
    ...
    fn test_order_full_fill() {
        ...
        let _ = engine.add_order_batch(vec![(
            1,
            1,
            Side::Sell,
            OrderType::Limit,
    ...
    fn test_order_partial_fill() {
        ...
        let (_res, commands) =
            engine.add_order_batch(vec![(1, 2, Side::Buy, OrderType::Limit, 50000, 500, 101, 200)]);
    ...
    fn test_order_status_sequence_new_before_match() {
        ...
        // 1. Maker Order (Sell)
        let (_res, maker_cmds) = engine.add_order_batch(vec![(
            1,
            1,
            Side::Sell,
            OrderType::Limit,
        ...
        assert!(
            maker_cmds.iter().any(
                |c| matches!(c, LedgerCommand::OrderUpdate(u) if u.status == OrderStatus::New)
            ),
            "Maker should emit New"
        );

        // 2. Taker Order (Buy)
        let (_res, taker_cmds) = engine.add_order_batch(vec![(
            1,
            2,
            Side::Buy,
            OrderType::Limit,
        ...
        let new_idx = taker_cmds
            .iter()
            .position(
                |c| matches!(c, LedgerCommand::OrderUpdate(u) if u.status == OrderStatus::New),
            )
            .expect("Taker should emit New");
        let match_idx = taker_cmds
            .iter()
            .position(|c| matches!(c, LedgerCommand::MatchExecBatch(_)))
            .expect("Taker should match");
    ...
    fn test_order_cancellation() {
        ...
        // Place Order
        let (res, _) = engine.add_order_batch(vec![(
            1,
            100,
            Side::Buy,
            OrderType::Limit,
