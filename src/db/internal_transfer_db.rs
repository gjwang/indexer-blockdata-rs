...
pub struct TransferRequestRecord {
    pub request_id: i64,
    pub user_id: i64, // Primary user context
...
const INSERT_TRANSFER_CQL: &str = "
    INSERT INTO trading.internal_transfers (
        request_id, user_id, from_account_type, from_user_id, to_account_type, to_user_id,
        asset_id, amount, status, created_at, updated_at, error_message
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
";

const GET_TRANSFER_BY_ID_CQL: &str = "
    SELECT request_id, user_id, from_account_type, from_user_id, to_account_type, to_user_id,
           asset_id, amount, status, created_at, updated_at, error_message
    FROM trading.internal_transfers
    WHERE request_id = ?
";
...
impl InternalTransferDb {
    ...
    /// Update transfer status (simple version)
    pub async fn update_transfer_status(
        ...
        let query = "UPDATE trading.internal_transfers SET status = ?, updated_at = ?, error_message = ? WHERE request_id = ?";

        self.session
            .query(query, (status, now, error_message.unwrap_or_default(), request_id))
            .await
    ...
    pub async fn get_transfer_by_id(&self, request_id: i64) -> Result<Option<TransferRequestRecord>> {
        let result = self.session
            .query(GET_TRANSFER_BY_ID_CQL, (request_id,))
            .await
